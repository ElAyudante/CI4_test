...
<div id="cont_2">
   <h1>Documentos de inter�s</h1>
    <?php 
		  ...
		  $SQL1="SELECT * FROM documentos WHERE Publico='0' OR Publico='1' ORDER BY Id DESC";
		  ...
		  if ($num_res1<1) {
			echo "No se han encontrado documentos."; } // fin IF mysql_num_rows($SQL1)<0		
			  ...
			  while ($campo=mysql_fetch_array($res_con1)) {
				  ...
				  <a href='download.php?ref=".$campo["Id"]."'>Ver documento</a>
				  </td>
				  </tr>";
	...
	?>
    <p>&nbsp;</p>
    <h1>Convenios firmados</h1>
    
    <?php 
		
		  $SQL1="SELECT * FROM documentos WHERE Publico='2' ORDER BY Id DESC";
		  $res_con1=mysql_query($SQL1) or die ("Intentalo m�s tarde. Ha habido un problema con la base de datos. Disculpa las molestias");
		  $num_res1=mysql_num_rows($res_con1);
		  if ($num_res1<1) {
			echo "No se han encontrado documentos."; } // fin IF mysql_num_rows($SQL1)<0		
