    ...
    <?php
		// obtengo el precio de las cuotas
			...
			$importe_cuotas=mysql_fetch_array($con_cuotas);
	  		...
	  		$sql_ecu="SELECT colegiados.Apellidos, colegiados.Ejerciente, cobroscuotas.*, estadocobrocuotas.Estado FROM colegiados, cobroscuotas, estadocobrocuotas WHERE colegiados.Id=".$_SESSION['id_usuario']." AND estadocobrocuotas.relColegiado='".$_SESSION['id_usuario']."' AND cobroscuotas.Id=estadocobrocuotas.relCuota ORDER BY cobroscuotas.Id";
			...
			if(mysql_num_rows($con_ecu)>0) {					
				...
				while($estado=mysql_fetch_array($con_ecu)) {
					...
					if($estado["Ejerciente"]==1) echo " (Importe:".$importe_cuotas["Ordinaria"]." �)"; else echo " (Importe:".$importe_cuotas["NoEjerciente"]." �)";
					echo "</td>\n";
