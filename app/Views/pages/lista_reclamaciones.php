...
<div id="cont_2">
   <h1>Lista de reclamaciones</h1>
    <?php 
		  ...
		  $SQL1="SELECT * FROM reclamaciones ORDER BY Id DESC";
		  ...
		  if ($num_res1<1) {
			echo "No se han encontrado reclamaciones."; } // fin IF mysql_num_rows($SQL1)<0		
			  ...
			  while ($campo=mysql_fetch_array($res_con1)) {
				  ...
				  if($campo["Respuesta"]==0){
				  	echo "<a href='responder_reclamacion.php?id=".$campo["Id"]."'>Responder</a>";
				  }
				  else {
					  echo "<a href='responder_reclamacion.php?id=".$campo["Id"]."'>Ver respuesta</a>";
				  }
					
					echo"&nbsp;&nbsp;<a href='borrar_reclamaciones.php?id=".$campo["Id"]."' onclick=\"return confirm('�Est� seguro de que desea eliminar esta reclamaci�n?')\">Eliminar</a>
				  </td>
				  </tr>";
