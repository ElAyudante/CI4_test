<?php session_start();
	...
	function Footer() {	 	  
		...
		$pdf->Cell(3);	
		$pdf->Cell(80,6,'Sociedad','B','','L');
		$pdf->Cell(17,6,'N� Colegiado','B','','R');
		$pdf->Cell(17,6,'Localidad','B','','R');
		$pdf->Cell(73,6,'Tipo Sociedad','B','','R');
		...
		$pdf->MultiCell(80,4,$rped["Sociedad"],0,'L');	
		
		$yy=$pdf->GetY();
		$pdf->SetXY($x+80,$y);	
		
		$pdf->Cell(17,3,$rped["Colegiado"],0,'','C');
		$pdf->Cell(17,3,$rped["Poblacion"],0,'','R');
		$pdf->Cell(73,3,$rped["TipoSociedad"],0,'','R');
		$pdf->SetY($yy);
		...
		$pdf->Output('../presupuestos/listado_sociedades.pdf', 'I');
