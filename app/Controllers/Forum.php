...
class Forum extends CI_Controller {
		...
		if ($slug === false) {
			...
			foreach ($forums as $forum) {
				...
				if ($forum->count_topics > 0) {
					...
					$forum->latest_topic->author    = $this->user_model->get_user_username_from_id($forum->latest_topic->user_id);
			...
			foreach ($topics as $topic) {
				...
				$topic->author                  = $this->user_model->get_user_username_from_id($topic->user_id);
				...
				$topic->latest_post->author     = $this->user_model->get_user_username_from_id($topic->latest_post->user_id);
	...
	public function create_forum() {
		...
		$this->form_validation->set_rules('title', 'Forum Title', 'trim|required|alpha_numeric_spaces|min_length[4]|max_length[255]|is_unique[forums.title]', array('is_unique' => 'El título del foro que ha introducido ya existe. Por favor, introduzca uno nuevo.'));
		...
		if ($this->form_validation->run() === false) {
			...
			echo view('templates/header');
			echo view('forum/create/create', $data);
			echo view('templates/footer');
			...
			if ($this->forum_model->create_forum($title, $description)) {
				...
				// forum creation ok
				echo view('templates/header');
				echo view('forum/create/create_success', $data);
				echo view('templates/footer');
				
			} else {
				
				// forum creation failed, this should never happen
				$data->error = 'Hubo un problema creando el nuevo foro. Por favor, inténtelo de nuevo.';
	...
	public function create_topic() {
		...
		// $breadcrumb .= '<li class="active">Crear un nuevo tema</li>';
		// $breadcrumb .= '</ol>';
		...
		// set validation rules
		$this->form_validation->set_rules('title', 'Topic Title', 'trim|required|alpha_numeric_spaces|min_length[4]|max_length[255]|is_unique[topics.title]', array('is_unique' => 'El tema que ha introducido ya existe. Por favor, escoja uno distinto.'));
		...
		if ($this->form_validation->run() === false) {
			...
			// validation not ok, send validation errors to the view
			echo view('templates/header');
			echo view('topic/create/create', $data);
			echo view('templates/footer');
			...
			if ($this->forum_model->create_topic($forum_id, $title, $content, $user_id)) {
				...
				// topic creation ok
				return redirect()->to(base_url('topic/' . $forum_slug . '/' . strtolower(url_title($title))));
				
			} else {
				
				// topic creation failed, this should never happen
				$data->error = 'Hubo un problema creando el nuevo tema. Por favor, inténtelo de nuevo.';
	...
	public function topic($forum_slug, $topic_slug) {
		...
		foreach ($posts as $post) {
			...
			$post->author = $this->user_model->get_user_username_from_id($post->user_id);
	...
	public function create_post($forum_slug, $topic_slug) {
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->forum_model->create_post($topic_id, $user_id, $content)) {
				...
				// post creation ok
				return redirect()->to(base_url('topic/' . $forum_slug . '/' . $topic_slug));
				
			} else {
				
				// post creation failed, this should never happen
				$data->error = 'Hubo un problema creando su respuesta. Por favor, inténtelo de nuevo.';
